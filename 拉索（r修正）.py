...
def lasso_r_squared(y_true, y_pred, y_mean, lambda_, beta):
    """
    计算拉索回归的修正R²
    """
    # 计算惩罚项（L1范数，不包括截距）
    penalty = lambda_ * np.sum(np.abs(beta[1:]))  # 不包括截距
...
def train_arimax_lasso(y, X, lag, alphas=np.logspace(-3, 3, 100)):
    """
    使用拉索回归（自动选择最佳alpha），并计算相关统计量和F检验。
    
    参数:
    y: 因变量 (Pandas Series)
    X: 自变量 (Pandas DataFrame)
    lag: 滞后阶数 (整数)
    alphas: 拉索回归的候选正则化参数列表 (array-like)
    ...
    lasso_cv.fit(X_matrix, y_matrix)
    best_alpha = lasso_cv.alpha_
    
    # 使用最佳 alpha 拟合 Lasso 回归
    lasso = LassoCV(alphas=[best_alpha], cv=5, max_iter=10000, random_state=0)
    lasso.fit(X_matrix, y_matrix)
    coef = lasso.coef_ 
    
    # 预测
    y_pred = lasso.predict(X_matrix)
    
    # 计算修正的R²
    y_mean = np.mean(y_matrix)
    r_squared_modified = lasso_r_squared(y_matrix, y_pred, y_mean, best_alpha, coef)
    ...
    # 构建结果字典，添加最佳alpha和F检验
    result = {
        'Lag': lag,
        'Best_alpha': best_alpha,
        'R_squared': r_squared_modified,
...
for lag in lags:
    try:
        res = train_arimax_lasso(y, X, lag)  # 使用自动选择的 alphas
        results.append(res)
...
for res in results:
    ...
    coef_names = ['const'] + list(X.columns)  # 确保列名和系数一一对应
    
    # 确保系数和列名长度匹配
    assert len(coef) == len(coef_names), f"系数数量({len(coef)})与列名数量({len(coef_names)})不匹配"
...
save_path = os.path.join(desktop_path, '拉索全部（r修正）.xlsx')
